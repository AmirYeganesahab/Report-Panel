...
class Graph(QDialog):
    ...
    def generate_plot(self, data, labels, plot_type='bar'):
        ...
        if plot_type == 'bar':
            ...
            # plot data
            patches, _, _ = ax.pie(
                data, labels=labels, shadow=False, autopct='%1.f%%', startangle=0)  # refresh canvas
            legend = ax.legend(
                patches, labels, bbox_to_anchor=(1.5, 0.5), loc='right')
...
class ComparisonWindow(QMainWindow):
    ...
    def file_save(self):
        ...
        try:
            ...
            MessageBox(
                title='File Saved', message=f'table and pie chart saved at {os.path.join(*pngName_left.split("/")[:-1])}')
    ...
    def constructFrames(self):
        ...
        (label1l, label2l, label3l), (self.left_pannel.combo1, self.left_pannel.combo2,
                                      self.left_pannel.combo3), _ = self.__panel__(side='left')
        ...
        (label1r, label2r, label3r), (self.right_pannel.combo1, self.right_pannel.combo2,
                                      self.right_pannel.combo3), (self.left_pannel.box, self.right_pannel.box) = self.__panel__(side='right')
    ...
    def populate_right_combo4(self, text=None):
        ...
        try:
            ...
            self.right_pannel.combo4.addItems(
                [str(d) for d in first_column[0]])
    ...
    def get_query_entity_from_right_combo23(self, text=None):
        ...
        # construct dataframe
        self.df_right = pd.DataFrame({'Row Labels': self.rlabels, 'Count': self.ry, 'percentages': [
                                     "{:.0%}".format(round(np.sum(p))/100) for p in self.rpercentages]})
        self.df_right = pd.concat([self.df_right, pd.DataFrame.from_records(
            [{'Row Labels': 'Grand Total', 'Count': s, 'percentages': "{:.0%}".format(int(round(np.sum(self.rpercentages)))/100)}])])
        try:
    ...
    def get_query_entity_from_left_combo23(self, text=None):
        ...
        try:
            ...
            # construct dataframe
            self.df_left = pd.DataFrame({'Row Labels': self.llabels, 'Count': self.ly, 'percentages': [
                                        "{:.0%}".format(round(np.sum(p))/100) for p in self.lpercentages]})
            self.df_left = pd.concat([self.df_left, pd.DataFrame.from_records(
                [{'Row Labels': 'Grand Total', 'Count': s, 'percentages': "{:.0%}".format(int(round(np.sum(self.lpercentages)))/100)}])])
    ...
    def create_left_chart(self):
        ...
        if self.left_radiob.b1.isChecked():
            ...
            plot_type = 'pie'
        self.left_pannel.graph.generate_plot(
            labels=self.llabels, data=percentages, plot_type=plot_type)
    ...
    def create_right_chart(self):
        ...
        self.right_pannel.graph.generate_plot(
            labels=self.rlabels, data=percentages, plot_type=plot_type)
    ...
    def create_left_table(self):
        try:
            ...
            self.left_pannel.reporttableWidget.setHorizontalHeaderLabels(
                self.df_left.columns)
            ...
            for i, col in enumerate(self.df_left.columns):
                for j, d in enumerate(self.df_left[col]):
                    ...
                    try:
                        self.left_pannel.reporttableWidget.setItem(
    ...
    def create_right_table(self):
        try:
            ...
            self.right_pannel.reporttableWidget.setHorizontalHeaderLabels(
                self.df_right.columns)
            ...
            for i, col in enumerate(self.df_right.columns):
                for j, d in enumerate(self.df_right[col]):
                    if type(d) == pd._libs.tslibs.timestamps.Timestamp:
                        d = pd.Timestamp.strftime(d, '%Y-%m-%d %X')
                    try:
                        self.right_pannel.reporttableWidget.setItem(
                            j, i, QTableWidgetItem(str(d)))
                    except Exception as e:
                        print('____________________')
                        print(e)
